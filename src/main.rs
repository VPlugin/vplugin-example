...
fn main() {
    ...
    let (mut window, recv) = glfw
        .create_window(1024, 768, "VPlugin Example", glfw::WindowMode::Windowed)
        .expect("Couldn't create window");
    window.make_current();
    window.set_key_polling(true);

    /*
     * We have to declare a seperate scope in this case. However that's just to make rustc happy, and
     * doesn't matter at all for VPlugin itself.
     */
    {
        /*
        ...
        let mut plugin_index = 0; // keeps track of the loaded plugins.
         ...
         */
        {
            for plugin in &mut plugins {
                plug_mgr
                    .begin_plugin(plugin)
                    .expect("Couldn't start plugin.");

                let redraw_hook =
                    plug_mgr.get_custom_hook::<(), ()>(&plugin, "vplugin_example_on_redraw");
